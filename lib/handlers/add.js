...
module.exports = function (select, textOnly, execute) {
  return {
    department: async () => {
      ...
      const message = `${answers.name} is to departments.`
    ...
    role: async () => {
      ...
      const message = `${
        answers.title
      } is as a role in the ${answers.department.match(
        textOnly
      )} department.`
      return execute(sql, message)
    },
    ...
    employee: async () => {
      ...
      const message = `${answers.firstName} ${answers.lastName} is to employees.`
      return execute(sql, message)
    },
  }
}
